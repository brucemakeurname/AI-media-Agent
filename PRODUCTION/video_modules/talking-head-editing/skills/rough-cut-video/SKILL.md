...
> **Known risk (BUG-011, not yet fixed here):** for `language: vi`, this CLI's default forced-alignment (separate wav2vec2 Vietnamese model) has been confirmed to silently collapse word timestamps on real narration audio in a sibling pipeline. If word `score` values come back near-zero for a run of words, or subtitles built from this transcript look desynced, see `docs/debug/bug-codebook/BUG-011-whisperx-align-timestamp-collapse.md` before debugging further.
...
> Same BUG-011 risk as Step 1 — this is the transcript `subtitle-designer` (Phase 4) builds subtitles from, so a timestamp collapse here directly desyncs the final subtitles.
