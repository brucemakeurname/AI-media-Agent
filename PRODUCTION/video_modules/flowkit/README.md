...
> **Solo Flows fork notice (2026-08-07):** This copy diverges from upstream
> (`tuannguyenhoangit-droid/google-flow-agent`) with Solo Flows-added features tracking Google
> Flow's own platform updates as of that date — most notably **Omni Flash** video generation
> (`abra_t2v_*`/`abra_r2v_*`, 4 discrete durations: 4/6/8/10s), which upstream's model set never
> called (upstream only wires Veo 3.1). Also corrects an upstream doc bug: **1080p upscale works
> on `PAYGATE_TIER_ONE`**, not TIER_TWO-only as previously documented. See
> `skills/fk-omni-video-gen.md` for usage and `docs/omni-discovery-log.md` for the full
> reverse-engineering trail (raw requests/responses). Reference-voice (`referenceAudioIds`) was
> investigated but found NOT live on Google's backend as of this date — documented as unconfirmed,
...
7. (Optional) Upscale  POST /api/requests {type: UPSCALE_VIDEO} (1080p verified on TIER_ONE 2026-08-07; 4K tier requirement unconfirmed)
...
| `/fk-gen-videos` | Generate videos from scene images (upscale via `UPSCALE_VIDEO` request — 1080p works on `PAYGATE_TIER_ONE`, 4K unconfirmed) |
| `/fk-omni-video-gen` | **Solo Flows fork addition (2026-08-07)** — Omni Flash text/reference-to-video, durations 4/6/8/10s |
...
| **Upscale** | `UPSCALE_VIDEO` | Video → 1080p (verified TIER_ONE 2026-08-07) or 4K (tier requirement unconfirmed) |
...
| `PUBLIC_ERROR_MODEL_ACCESS_DENIED` | Tier mismatch (e.g. TIER_ONE trying Veo 3 Ultra, or 4K upscale) — NOT 1080p upscale, that works on TIER_ONE (verified 2026-08-07) | Mark FAILED — auto-detect should downgrade to allowed model |
...
| Upscale "permission denied" | Not a hard TIER_ONE block for 1080p — verified working 2026-08-07. If it still fails, check tier/model mismatch generally (`/api/flow/credits`); 4K may still require `PAYGATE_TIER_TWO` |
