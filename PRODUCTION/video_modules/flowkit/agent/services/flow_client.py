...
class FlowClient:
    ...
    async def generate_video_refs_omni(self, reference_media_ids: list[str],
                                        prompt: str, project_id: str, scene_id: str,
                                        aspect_ratio: str = "VIDEO_ASPECT_RATIO_PORTRAIT",
                                        duration_s: int = 4,
                                        user_paygate_tier: str = "PAYGATE_TIER_ONE") -> dict:
        """Generate video via Omni Flash (reference-to-video, character/product consistency).

        Model key pattern `abra_r2v_{duration_s}s` reverse-engineered 2026-08-07 the same way as
        `abra_t2v_4s` (see generate_video_text). `duration_s` in {4, 6, 8, 10} — all four
        live-verified working on PAYGATE_TIER_ONE (credit cost rises with duration: ~10/12/15
        credits observed for 6s/8s/10s). Same endpoint as the Veo reference method
        (`generate_video_references`/`batchAsyncGenerateVideoReferenceImages`) — kept as a
        separate method (not a param on generate_video_from_references) so existing Veo-based
        callers are untouched.
        """
        if duration_s not in (4, 6, 8, 10):
            return {"error": f"duration_s must be one of 4, 6, 8, 10 (got {duration_s})"}
        model_key = f"abra_r2v_{duration_s}s"
    ...
    async def generate_video_text(self, prompt: str, project_id: str, scene_id: str,
                                   ...
                                   duration_s: int = 4,
                                   user_paygate_tier: str = "PAYGATE_TIER_ONE") -> dict:
        """Generate video via Omni Flash (text-to-video, no image input).

        Model key pattern `abra_t2v_{duration_s}s` and endpoint `batchAsyncGenerateVideoText`
        reverse-engineered 2026-08-07 by sniffing Flow's own web UI network traffic
        (VIDEO_MODULES/flowkit's own model set never used this path — it was image-anchored
        generation only). `duration_s` in {4, 6, 8, 10} — all four live-verified working on
        PAYGATE_TIER_ONE (credit cost rises with duration: ~10/12/15 credits observed for
        6s/8s/10s vs the 4s baseline). The short-form "PORTRAIT" value seen in Flow's frontend
        analytics event (batchLogFrontendEvents) is NOT what the real API field expects —
        confirmed by a live 400 (INVALID_ARGUMENT on requests[0].aspect_ratio) — the actual
        request needs the same "VIDEO_ASPECT_RATIO_*" enum as every other video-gen endpoint.
        """
        if duration_s not in (4, 6, 8, 10):
            return {"error": f"duration_s must be one of 4, 6, 8, 10 (got {duration_s})"}
