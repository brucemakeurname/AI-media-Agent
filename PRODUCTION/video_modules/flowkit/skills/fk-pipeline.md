...
Options:
- `--upscale` — include upscale stage. **Corrected 2026-08-07:** 1080p upscale (`veo_3_1_upsampler_1080p`) live-verified working on `PAYGATE_TIER_ONE`, no credit cost seen. 4K upscale (`veo_3_1_upsampler_4k`) tier requirement not re-verified — treat as TIER_TWO-only until confirmed otherwise
...
Only run after review passes (or max review cycles exhausted). 1080p works on TIER_ONE
(verified 2026-08-07 — see `fk-doctor.md`); 4K tier requirement unconfirmed.
...
| Upscale TIER_ONE error on 1080p | Unexpected — 1080p verified working on TIER_ONE 2026-08-07, check `/fk-doctor` for a different cause |
| Upscale TIER_ONE error on 4K | Likely still TIER_TWO-gated — try 1080p (`resolution: VIDEO_RESOLUTION_1080P`) instead |
