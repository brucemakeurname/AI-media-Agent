...
| Upscale "permission denied" | **Corrected 2026-08-07:** this is NOT a hard TIER_ONE block — live-verified `POST /api/flow/upscale-video` (`veo_3_1_upsampler_1080p`) succeeds on `PAYGATE_TIER_ONE` with no credit cost seen. If you still hit `PUBLIC_ERROR_MODEL_ACCESS_DENIED` on upscale, treat it as the generic tier/model-mismatch case above (`GET /api/flow/credits`, check `models.json`), not an automatic TIER_TWO requirement |
