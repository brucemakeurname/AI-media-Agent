...
## 2026-08-07 Solo Flows fork — Omni Flash + doc corrections

This copy diverges from upstream as of this date. Full raw evidence trail in
`docs/omni-discovery-log.md`; usage doc in `skills/fk-omni-video-gen.md`. Summary:

- **Omni Flash video model added.** Upstream's `models.json`/`FlowClient` only ever called Veo
  3.1 endpoints (`batchAsyncGenerateVideo{StartImage,StartAndEndImage,ReferenceImages}`). Flow's
  own web UI also exposes an "Omni" model (internal codename `abra`) via two endpoints upstream
  never used: `batchAsyncGenerateVideoText` (text-to-video, model `abra_t2v_{N}s`) and the
  existing `batchAsyncGenerateVideoReferenceImages` endpoint with a different model key
  (`abra_r2v_{N}s`) than Veo's r2v. `N` ∈ {4, 6, 8, 10} seconds — all four confirmed live on
  `PAYGATE_TIER_ONE`. New methods: `FlowClient.generate_video_text`,
  `FlowClient.generate_video_refs_omni`. New routes: `POST /api/flow/generate-video-text`,
  `POST /api/flow/generate-video-refs-omni`.
- **Discovery method: passive native-traffic sniffing**, not blind API guessing. A
  `chrome.webRequest.onBeforeRequest` listener (separate from the pre-existing token-capture
  `onBeforeSendHeaders` one) in `extension/background.js` observes ALL requests to
  `aisandbox-pa.googleapis.com`, including ones Flow's own website fires natively — not just
  agent-proxied ones. This is how the real model keys/endpoints were found instead of reverse
  engineering from documentation that doesn't exist for a Public Preview feature.
- **Upstream doc bug corrected: 1080p upscale works on `PAYGATE_TIER_ONE`.** `fk-doctor.md`,
  `fk-pipeline.md`, and `README.md` all claimed upscale requires `PAYGATE_TIER_TWO`. Live-tested
  false — `veo_3_1_upsampler_1080p` succeeds on TIER_ONE, no extra credit cost observed. 4K
  upscale tier requirement was NOT re-tested and remains presumed TIER_TWO-only.
- **`POST /api/flow/raw` dev endpoint added** — thin passthrough to `FlowClient._send()` for
  probing undocumented endpoints without writing a typed method first. Used to investigate
  reference-voice (`referenceAudioIds`) support; conclusion was NOT live on Google's backend as
  of this date (3 field-placement probes all rejected — see discovery log for exact errors). This
  is a genuine gap in Google's own API, not something client-side code in this repo can add.
