...
// ─── Native Flow Request Sniffer (Solo Flows fork, added 2026-08-07) ────────
// Passively observes ALL requests to Flow's API domain — including ones the
// Flow website itself fires natively from the UI (model picker etc.), not just
// ones this agent proxies. This is how Omni Flash (abra_t2v_*/abra_r2v_*) was
// discovered — see docs/omni-discovery-log.md. Kept as a standing capability
// for future reverse-engineering of new Flow features, not a one-off script.

let nativeRequestLog = [];
...
chrome.webRequest.onBeforeRequest.addListener(
  (details) => {
    ...
    try {
      ...
      bodyStr = `<decode error: ${e.message}>`;
    }
    ...
    nativeRequestLog.unshift(entry);
    if (nativeRequestLog.length > 50) nativeRequestLog.pop();
    chrome.storage.local.set({ nativeRequestLog });

    // Audio/voice/likeness endpoints are small (no recaptcha token bloat) —
    // log full body untruncated so we can read the real field names.
    const isAudioRelated = /audio|voice|likeness/i.test(details.url);
    let focus = bodyStr;
    if (isAudioRelated) {
      focus = bodyStr; // full, untruncated
    } else if (bodyStr) {
      const idx = bodyStr.indexOf('modelKey');
      if (idx >= 0) {
        focus = '...' + bodyStr.slice(Math.max(0, idx - 40), idx + 400);
      } else if (bodyStr.length > 400) {
        focus = bodyStr.slice(0, 400) + '...(no modelKey found in first 400)';
      }
    }
