...
export interface BgmMixOptions {
  /** BGM volume relative to voice (linear, e.g. 0.12 ≈ -18 dB under voice) */
  bgmVolume: number;
  /** Fade in/out duration in seconds */
  fadeSec: number;
  /** When true, BGM is sidechain-compressed by voice (true ducking). */
  sidechainDuck: boolean;
}

/**
 * Mix a BGM track under voice.mp3, producing a new mixed track.
 *
 * - Voice stays at full volume
 * - BGM is looped/trimmed to match voice length
 * - BGM fades in over `fadeSec` at start, fades out over `fadeSec` at end
 * - When `sidechainDuck` is true, BGM is dynamically attenuated whenever
 *   voice has signal (true broadcast-style ducking).
 * - Output is mp3 at 192kbps
 */
export async function mixBgmUnderVoice(
  voicePath: string,
  bgmPath: string,
  outPath: string,
  ...
  const fade = Math.max(0.1, opts.fadeSec);
  const fadeOutSt = Math.max(0, voiceDurationSec - fade);

  // BGM chain: stereo → mono, loop forever, trim to voice length, volume, fade in/out
  const bgmChain =
    `[1:a]aresample=44100,aformat=sample_fmts=fltp:channel_layouts=mono,` +
    `aloop=loop=-1:size=2e9,atrim=duration=${voiceDurationSec.toFixed(3)},` +
    `volume=${opts.bgmVolume},` +
    `afade=t=in:st=0:d=${fade},afade=t=out:st=${fadeOutSt.toFixed(3)}:d=${fade}[bgm]`;

  const voiceChain = `[0:a]aresample=44100,aformat=sample_fmts=fltp:channel_layouts=mono[voice]`;

  let mixFilter: string;
  if (opts.sidechainDuck) {
    // Split voice into two: one for output, one as sidechain trigger.
    // Sidechain compresses BGM whenever voice has signal.
    mixFilter =
      `[voice]asplit=2[voiceOut][voiceSc];` +
      `[bgm][voiceSc]sidechaincompress=threshold=0.04:ratio=8:attack=20:release=300:makeup=1[bgmDucked];` +
      `[voiceOut][bgmDucked]amix=inputs=2:duration=first:dropout_transition=0:normalize=0[out]`;
  } else {
    mixFilter = `[voice][bgm]amix=inputs=2:duration=first:dropout_transition=0:normalize=0[out]`;
  }

  const filterGraph = `${voiceChain};${bgmChain};${mixFilter}`;
