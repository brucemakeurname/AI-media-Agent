...
export const KEYWORD_RULES: { pattern: RegExp; category: string; label: string }[] = [
  ...
  {
    pattern:
      /(cảnh báo|rủi ro|nguy hiểm|đáng lo|đe dọa|cảnh giác|tiêu cực|lo ngại|warning|danger|alert|risk|threat)/i,
    category: "alert",
    label: "warning",
  },
  ...
  {
    pattern:
      /(thất bại|sai lầm|sụp đổ|lỗi nghiêm trọng|không đạt|trượt|fail|error|wrong|mistake|crash|broken)/i,
    category: "fail",
    label: "failure",
  },
  ...
  {
    pattern:
      /(kỷ lục|kỉ lục|vượt xa|xuất sắc|đạt mốc|thành công|tăng mạnh|đột phá|hàng đầu|breakthrough|achievement|success|record|win|outperform)/i,
    category: "success",
    label: "success",
  },
  ...
  {
    pattern:
      /(tiết lộ|khám phá|lần đầu|công bố|ra mắt|trình làng|hé lộ|phát hành|reveal|launch|unveil|debut|announce|introduce)/i,
    category: "reveal",
    label: "reveal",
  },
  ...
  {
    pattern:
      /(hùng vĩ|hoành tráng|vĩ đại|chấn động|khổng lồ|cinematic|epic|massive|huge|colossal)/i,
    category: "cinematic",
    label: "cinematic",
  },
...
export function defaultPlayback(picked: PickedSfx): { volume: number; offsetSec: number } {
  ...
  switch (cat) {
    case "transition":
      return { volume: 0.4, offsetSec: 0.0 };
    case "emphasis":
      return { volume: 0.35, offsetSec: 0.2 };
    case "alert":
      return { volume: 0.4, offsetSec: 0.1 };
    case "success":
      return { volume: 0.35, offsetSec: 0.3 };
    case "fail":
      return { volume: 0.35, offsetSec: 0.1 };
    case "reveal":
      return { volume: 0.3, offsetSec: 0.2 };
    case "countdown":
      return { volume: 0.3, offsetSec: 0.0 };
    case "cinematic":
      return { volume: 0.35, offsetSec: 0.0 };
    case "drumroll":
      return { volume: 0.4, offsetSec: 0.0 };
    case "outro":
      return { volume: 0.35, offsetSec: 0.5 };
    default:
      return { volume: 0.35, offsetSec: 0.1 };
  }
}
