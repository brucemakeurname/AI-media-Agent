...
// Center X; Y at the lower third boundary (1/3 of frame height up from the
// bottom edge) — industry-news layout keeps captions clear of the
// upper hook/headline zone and just above the lower graphic band.
const SUB_X = Math.round(PLAY_RES_X / 2);
const SUB_Y = Math.round(PLAY_RES_Y * (2 / 3));
...
// Groups words into short display bursts (≤4 words, break on ≥0.6s gap) —
// same word-timing input/position philosophy as the original single-word
// style, just multiple words visible per burst so the active-word highlight
// (per caption-styles.json's animationType) has context to highlight against.
function groupWords(words: WordEntry[], maxWords = 4, gapBreakSec = 0.6): WordEntry[][] {
 ...
 * Burns styled word-highlight subtitles into `videoPath` → `outputPath`, using
 * one of the 6 presets in `caption-styles.json` (ported from
 * nicolaigaina/ai-video-captions' caption-styles.config.json).
 *
