...
description: Consume a locked node/shooting-script.md plus its Gemini TTS timing lock, retrieve and score candidate templates from BASE/BRAND KITs/5. Video_Prompt_Template, then write one 4/6/8/10s full-JSON Omni prompt per minimum-count sequence. Each prompt can contain timed sub-scenes, jumpcuts, transitions, and (for AI-clone work) accepted crawler keyframe refs. Part C locks reuse of the TTS audio and optional BGM. content-executive or designer, single pass.
---
...
4. **Preserve speech exactly.** `voice` is only the exact approved dialogue assigned to the sequence;
   ...
   Gemini TTS timing lock; this prevents prompt-metadata from corrupting the line used for TTS/subtitles.
...
Most raw UGC clips need **no separate BGM layer**. The pre-generated Gemini TTS timing-lock audio is
