...
console.log('loaded');
const result = await page.evaluate(() => { const scene=document.querySelector('.us-overlay'); const tl=gsap.timeline({paused:true}); window.SFV_ANIMATORS['product-pop-up'](scene,tl,0,4); return {duration:tl.duration(), children:tl.getChildren().length}; });
console.log(result);
await browser.close();
