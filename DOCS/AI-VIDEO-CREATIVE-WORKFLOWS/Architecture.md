...
4. Chạy `video-thumbnail` từ first beat/locked sequence trước khi dùng `acad-image-gen` render `thumbnail.jpg`.

### 3.3 Video Editor

1. Parse từng JSON block trong `ugc-sequence-script.md`; gửi **toàn bộ JSON** cùng tối đa 3 reference media IDs vào FlowKit Omni.
2. Nhận raw clip và bắt buộc gọi FlowKit upscale `VIDEO_RESOLUTION_1080P`; nếu lỗi thì dùng `ffmpeg-upscale-video` và log fallback.
3. Download từng scene rồi remove watermark bằng `gwt-remove-watermark-video` trước voice/audio hoặc concat.
4. Concat scene theo thứ tự, dead-air check, WhisperX + approved-text QA, burn subtitle khi ticket yêu cầu, mix BGM/SFX, rồi prepend `thumbnail.jpg` để **frame đầu tiên** của final MP4 là thumbnail.
